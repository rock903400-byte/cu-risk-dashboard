...
def render_war_room_page(df_csv: pd.DataFrame, is_admin: bool, config: dict):
    ...
    with st.sidebar:
        ...
        st.markdown(
            '<span class="sidebar-label">🔍 財報篩選</span>', unsafe_allow_html=True
        )
        ...
        selected_year = (
            st.selectbox("選擇年度", all_years, index=0) if all_years else None
        )

        year_months = (
            sorted(union_data[union_data["年度"] == selected_year]["年月"].unique())
            if selected_year
            else []
        )
        selected_month = (
            st.selectbox("選擇月份", year_months, index=len(year_months) - 1)
            if year_months
            else None
        )

        if selected_year:
            total_months = union_data[union_data["年度"] == selected_year][
                "年月"
            ].nunique()
    ...
    filtered = (
        union_df[
            (union_df["年月"] == selected_month) & (union_df["社名"] == selected_union)
        ]
        if selected_month
        else pd.DataFrame()
    )
        ...
        if filtered.empty:
            ...
            if show_compare and year_months:
                ...
                if curr_idx > 0:
                    ...
                    prev_month_data = union_df[
                        (union_df["年月"] == prev_month)
                        & (union_df["社名"] == selected_union)
                    ].copy()
                    prev_month_data["類別"] = prev_month_data["會計科目"].apply(
                        classify_code
                    )
                else:
                    st.info("無前期月份可對比。")
            ...
            if show_compare and not prev_month_data.empty:
                ...
                def _render_bs_with_compare(category_filter, title):
                    st.markdown(f"#### 【 {title} 】")
                    curr = bs_df[bs_df["類別"] == category_filter].sort_values(
                        "會計科目"
                    )[["會計科目", "會科名稱", "當月金額"]]
                    prev = (
                        prev_month_data[prev_month_data["類別"] == category_filter]
                        .sort_values("會計科目")[["會計科目", "當月金額"]]
    ...
    with tab_is:
        ...
        if not selected_year:
            ...
            is_annual = is_annual[
                is_annual["會計科目"].astype(str).str.match(r"^[45]")
            ].copy()
            ...
            if show_compare:
                prev_year = None
                if (
                    selected_year in all_years
                    and all_years.index(selected_year) < len(all_years) - 1
                ):
                    prev_year = all_years[all_years.index(selected_year) + 1]
                if prev_year:
                    curr_months_list = sorted(
                        is_df[is_df["年度"] == selected_year]["年月"].unique()
                    )
                    prev_months_list = sorted(
                        is_df[is_df["年度"] == prev_year]["年月"].unique()
                    )
                    prev_is_annual = get_annual_snapshot(
                        is_df, prev_year, same_months=curr_months_list
                    )
                    prev_is_annual = prev_is_annual[
                        prev_is_annual["會計科目"].astype(str).str.match(r"^[45]")
                    ].copy()
                    prev_is_annual["類別"] = prev_is_annual["會計科目"].apply(
                        classify_code
                    )
                    if len(curr_months_list) < len(prev_months_list):
                        st.info(
                            f"ℹ️ {selected_year}年僅有 {len(curr_months_list)} 個月資料，已自動取{prev_year}年同期 {len(curr_months_list)} 個月進行公平對比。"
                        )
                else:
                    st.info("無前期年度可對比。")
            ...
            if show_compare and not prev_is_annual.empty:
                prev_incomes = (
                    prev_is_annual[prev_is_annual["類別"] == "收入"]
                    .sort_values("會計科目")[["會計科目", "當月金額"]]
                    .rename(columns={"當月金額": "去年金額"})
                )
                prev_expenses = (
                    prev_is_annual[prev_is_annual["類別"] == "支出"]
                    .sort_values("會計科目")[["會計科目", "當月金額"]]
                    .rename(columns={"當月金額": "去年金額"})
                )

                inc_merged = pd.merge(
                    incomes, prev_incomes, on="會計科目", how="outer"
                ).fillna(0)
                inc_merged["增減"] = inc_merged["當月金額"] - inc_merged["去年金額"]
                inc_merged["增減率"] = inc_merged.apply(
                    lambda r: r["增減"] / r["去年金額"] if r["去年金額"] != 0 else None,
                    axis=1,
                )

                exp_merged = pd.merge(
                    expenses, prev_expenses, on="會計科目", how="outer"
                ).fillna(0)
                            ...
                            [
                                {
                                    ...
                                    "會科名稱": "營業收入合計",
                                    "今年金額": _fmt(rev_total),
                                    "去年金額": _fmt(prev_rev_total),
                                    "增減": _fmt(rev_delta),
                                    "增減率": (
                                        f"{rev_delta/prev_rev_total:+.1%}"
                                        if prev_rev_total
                                        else ""
                                    ),
                                }
                            ...
                            [
                                {
                                    ...
                                    "會科名稱": "營業支出合計",
                                    "今年金額": _fmt(exp_total),
                                    "去年金額": _fmt(prev_exp_total),
                                    "增減": _fmt(exp_delta),
                                    "增減率": (
                                        f"{exp_delta/prev_exp_total:+.1%}"
                                        if prev_exp_total
                                        else ""
                                    ),
                                }
                ...
                def style_is_compare(row):
                    ...
                    if "合計" in str(row["會科名稱"]):
                        styles = ["font-weight: bold; background-color: #f0f2f6"] * len(
                            row
                        )
                    elif "淨利" in str(row["會科名稱"]):
                        bg = "#DCFCE7" if net_profit >= 0 else "#FEE2E2"
                        styles = [
                            f"font-weight: bold; background-color: {bg}; border-top: 2px solid black"
                        ] * len(row)
                ...
                st.dataframe(
                    ...
                    .style.apply(style_is_compare, axis=1)
                    .set_properties(**{"font-size": "16px"}),
                    use_container_width=True,
                    hide_index=True,
                )
                ...
                def style_is(row):
                    if "合計" in str(row["會科名稱"]):
                        return ["font-weight: bold; background-color: #f0f2f6"] * len(
                            row
                        )
                ...
                st.dataframe(
                    is_disp.style.apply(style_is, axis=1).set_properties(
                        **{"font-size": "16px"}
                    ),
                    use_container_width=True,
                    hide_index=True,
    ...
    with tab_overview:
        ...
        if not selected_year:
            ...
            prev_agg = (
                get_annual_snapshot(
                    analysis_df, prev_year, same_months=curr_months_list
                )
                if prev_year
                else None
            )
    ...
    with tab_deep:
        ...
        if not selected_year:
            ...
            compare_options = ["（不比較）"] + [
                y for y in all_years if y != selected_year
            ]
            compare_choice = st.selectbox(
                "📅 對比年度（選填）", compare_options, key="deep_compare"
            )
            ...
            if compare_year:
                ...
                _cmp_months = sorted(
                    analysis_df[analysis_df["年度"] == compare_year]["年月"].unique()
                )
                ...
                if len(_common_months) < len(_sel_months) or len(_common_months) < len(
                    _cmp_months
                ):
                    st.info(
                        f"ℹ️ {selected_year}年有 {len(_sel_months)} 個月、{compare_year}年有 {len(_cmp_months)} 個月資料，已自動取同期 {len(_common_months)} 個月進行公平對比。"
                    )
                    annual_agg = get_annual_snapshot(
                        analysis_df, selected_year, same_months=_common_months
                    )
                    compare_agg = get_annual_snapshot(
                        analysis_df, compare_year, same_months=_common_months
                    )
            ...
            if prev_year and prev_agg is not None and not prev_agg.empty:
                prev_months_list = sorted(
                    analysis_df[analysis_df["年度"] == prev_year]["年月"].unique()
                )
                if len(curr_months_list) < len(prev_months_list):
                    st.info(
                        f"ℹ️ {selected_year}年僅有 {len(curr_months_list)} 個月資料，已自動取{prev_year}年同期 {len(curr_months_list)} 個月進行公平對比。"
                ...
                with st.spinner("偵測年度變動..."):
                    render_yoy_anomalies(
                        yoy_annual_agg, prev_agg, selected_year, prev_year
                    )
            else:
    ...
    with tab_diag:
        ...
        if not selected_year:
            ...
            if (
                ...
                prev_year = all_years[all_years.index(selected_year) + 1]
            curr_months_list = sorted(
                diag_df[diag_df["年度"] == selected_year]["年月"].unique()
            )
            prev_agg = (
                get_annual_snapshot(diag_df, prev_year, same_months=curr_months_list)
                if prev_year
                else None
            )
            ...
            if annual_agg.empty:
                ...
                with col2:
                    ...
                    delta_er = (
                        f"{(er - prev_ratios['equity_ratio']):+.1%}"
                        if prev_ratios
                        else None
                    )
                    st.metric(
                ...
                with col3:
                    ...
                    delta_xr = (
                        f"{(xr - prev_ratios['expense_ratio']):+.1%}"
                        if prev_ratios
                        else None
                    )
                    st.metric(
                ...
                with col4:
                    ...
                    delta_net = (
                        format_large_number(net - prev_net)
                    ...
                    consecutive_loss = (
                        prev_ratios is not None
                        and net < 0
                        and prev_ratios["net_income"] < 0
                    )
                    if consecutive_loss:
                        st.error(
                            "🚨 連續兩年虧損，財務壓力持續，建議緊急檢視業務結構。"
                        )
                    elif net < 0:
                        st.error(
                            "🚨 本年度虧損，建議檢視主要支出項目，評估增收節支方案。"
                        )
                    else:
                ...
                if trend_df.empty:
                    ...
                    def _net_color(val):
                        return (
                            "color: #166534"
                            if val >= 0
                            else "color: #991B1B; font-weight: bold"
                        )

                    st.dataframe(
                        trend_df.style.map(
                            lambda v: _cell_color(v, "expense_ratio"), subset=["開支比"]
                        )
                        .map(
                            lambda v: _cell_color(v, "avg_rate"),
                            subset=["加權平均利率"],
                        )
                    ...
                    if len(loss_years) >= 2:
                        st.error(
                            f"⚠️ 歷年中有 {len(loss_years)} 個年度開支比超過 100%（{', '.join(loss_years)}），請持續關注收支趨勢。"
                        )
    ...
    with tab_raw:
        ...
        if filtered.empty:
            ...
            st.dataframe(
                filtered.style.format({"當月金額": "{:,.0f}"}).set_properties(
                    **{"font-size": "16px"}
                ),
                use_container_width=True,
            )
