...
def render_overview_page(data: pd.DataFrame, df_m: pd.DataFrame, df_l: pd.DataFrame,
    ...
    with tab_ov:
        ...
        def render_card(title, key, cls, criteria_text, show_reasons=False):
            ...
            if show_reasons:
                ...
                body = " ".join(f'<span class="name-tag">{n}</span>' for n in names) or "無"
            st.markdown(
        ...
        with sc1: render_card(
            "🚨 特別關懷", "特別關懷", "hdr-red",
            f"以下 5 項觸發任 2 項：<br>① 連兩年虧損 ② 貸放比＜{T['high_risk_loan_ratio']:.0%}<br>③ 高逾放且惡化 ④ 人數連三年衰退 ⑤ 股金連三年衰退",
            show_reasons=False,
        )
    ...
    with tab_mx:
        ...
        st.markdown("---")
        st.markdown("#### 各狀態社別一覽")
        status_order = ["🚨 特別關懷", "⚠️ 流動性緊繃", "💤 資金閒置", "✅ 穩健模範", "📊 一般狀態"]
