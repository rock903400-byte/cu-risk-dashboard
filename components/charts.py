...
def render_waterfall(annual_agg: pd.DataFrame, selected_year: str, theme_bg: str):
    ...
    text_list = [
        _fmt(v) if m != "total" else _fmt(net) for v, m in zip(values, measures)
    ]
...
def render_yoy_anomalies(
    annual_agg: pd.DataFrame, prev_agg: pd.DataFrame, selected_year: str, prev_year: str
):
    """YoY 異常偵測"""
    ...
    st.info(
        f"📊 與 **{prev_year}年** 相比，共 **{len(anomalies)}** 個科目有顯著變動\n\n"
    ...
    st.dataframe(
        _build_result(anomalies.head(10))
        .style.format(_FMT)
    ...
    if len(anomalies) > 10:
        ...
        st.dataframe(
            _build_result(anomalies.iloc[10:])
            .style.format(_FMT)
            .apply(_style_row, axis=1)
            .set_properties(**{"font-size": "16px", "padding": "8px"}),
            use_container_width=True,
        ...
        with tab:
            ...
            if comp_rank is not None:
                ...
                fig = px.bar(
                    ...
                    text="金額標籤",
                    color_discrete_map={
                        year_label: cat_color,
                        compare_label: "#94A3B8",
                    },
...
def render_yearly_trend(analysis_df: pd.DataFrame, theme_bg: str):
    ...
    yearly = (
        analysis_df.groupby(["年度", "類別"])["當月金額"].sum().unstack(fill_value=0)
    )
    for cat in ["收入", "支出"]:
    ...
    fig.add_trace(
        go.Bar(
            name="總收入", x=yearly["年度"], y=yearly["收入"], marker_color="#10B981"
        ),
        secondary_y=False,
    )
    fig.add_trace(
        go.Bar(
            name="總支出", x=yearly["年度"], y=yearly["支出"], marker_color="#EF4444"
        ),
        secondary_y=False,
    )
    fig.add_trace(
