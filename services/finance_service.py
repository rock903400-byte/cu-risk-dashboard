...
def get_annual_snapshot(
    df: pd.DataFrame, year_str: str, same_months: list[str] | None = None
) -> pd.DataFrame:
    """
...
def calc_yoy_pct(current: float, previous: float) -> float | None:
    """計算 YoY 變動率；分母為 0、None、NaN 時回傳 None"""
...
def prepare_waterfall_data(annual_agg: pd.DataFrame) -> dict:
    ...
    """
    biz = annual_agg[annual_agg["會計科目"].str.match(r"^53")].sort_values(
        "當月金額", ascending=False
    )
    ...
    for g_name in w_data.index:
        if g_name.startswith(("2", "3", "4.", "5.", "6", "7", "8")):
            ...
            labels.append(clean)
            raw_amt = w_data[g_name]
            values.append(-abs(raw_amt))
    ...
    net = values[0] + sum(
        v for v, m in zip(values[1:], measures[1:]) if m == "relative"
    )
...
def detect_yoy_anomalies(
    ...
    """
    ann = annual_agg.groupby("會計科目", as_index=False).agg(
        {"會科名稱": "first", "當月金額": "sum"}
    )
