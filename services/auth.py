...
def handle_login(max_attempts: int):
    ...
    if entered == admin_pw:
        st.session_state.update(logged_in=True, role="admin",
                                assigned_region=None, assigned_union=None)
    elif entered in pws:
        info = pws[entered]
...
def render_login_page(max_attempts: int):
    ...
    with col:
        with st.container(border=True):
            st.markdown("<h2 class='responsive-h2' style='text-align:center;'>🏦 儲互社分析系統</h2>",
                        unsafe_allow_html=True)
            if st.session_state.get("preload_err"):
                st.markdown(f'<div class="alert-box alert-error">⚠️ 無法讀取雲端資料，請確認連結。<br><small>錯誤訊息: {st.session_state["preload_err"]}</small></div>',
                            unsafe_allow_html=True)
            if st.session_state["locked"]:
                st.markdown('<div class="alert-box alert-error">🔒 嘗試次數過多，請稍後再試。</div>',
                            unsafe_allow_html=True)
            else:
                ...
                if attempts > 0:
                    st.markdown(
                        ...
                        unsafe_allow_html=True)
                st.text_input("密碼", type="password", key="pwd_input",
                              label_visibility="collapsed", placeholder="請輸入密碼")
                st.button("🔓 登入系統", use_container_width=True,
                          on_click=handle_login, args=(max_attempts,))
