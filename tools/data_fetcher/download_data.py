...
def get_credentials():
    ...
    class LoginDialog(simpledialog.Dialog):
        def body(self, master):
            self.title("請輸入登入資訊")
            tk.Label(master, text="帳號:").grid(row=0)
            tk.Label(master, text="密碼:").grid(row=1)
...
def get_aspset_payload_pairs(html, target_button_name=None, target_button_value=None):
    ...
    if target_button_value and not target_button_name:
        btn_match = re.search(
            r'<input[^>]+name=["\']([^"\']+)["\'][^>]+value=["\']'
            + re.escape(target_button_value)
            + r'["\']',
            html,
            re.I,
        )
        if btn_match:
            target_button_name = btn_match.group(1)

    for match in re.finditer(r"<input[^>]*>", html, re.I):
        tag = match.group(0)
        name_m = re.search(r'name=["\']?([^"\'\s>]+)', tag, re.I)
        type_m = re.search(r'type=["\']?([^"\'\s>]+)', tag, re.I)
        val_m = re.search(r'value=["\']([^"\']*)["\']', tag, re.I)
        checked = re.search(r"\schecked\b", tag, re.I)
        if not name_m:
            continue
        name = name_m.group(1)
        value = val_m.group(1) if val_m else "on"
        itype = type_m.group(1).lower() if type_m else "text"
        if itype == "hidden" or name.startswith("__"):
            pairs.append((name, val_m.group(1) if val_m else ""))
        elif itype == "checkbox" or itype == "radio":
            if checked:
                pairs.append((name, value))
        elif itype == "submit" or itype == "image":
            if name == target_button_name:
                pairs.append((name, target_button_value or value))
        else:
            pairs.append((name, val_m.group(1) if val_m else ""))

    for match in re.finditer(r"<select[^>]*>.*?</select>", html, re.I | re.S):
        select_tag = match.group(0)
        name_m = re.search(r'name=["\']?([^"\'\s>]+)', select_tag, re.I)
        if not name_m:
            continue
        name = name_m.group(1)
        found_any = False
        for opt_match in re.finditer(
            r'<option[^>]*selected[^>]*value=["\']?([^"\'\s>]+)["\']?', select_tag, re.I
        ):
            pairs.append((name, opt_match.group(1)))
            found_any = True
        if not found_any and "multiple" not in select_tag.lower():
            first_val_m = re.search(
                r'<option[^>]*value=["\']?([^"\'\s>]+)["\']?', select_tag, re.I
            )
            if first_val_m:
                pairs.append((name, first_val_m.group(1)))
...
def do_postback(opener, url, current_html, target, argument, headers, overrides=None):
    ...
    payload.update({"__EVENTTARGET": target, "__EVENTARGUMENT": argument})
    if overrides:
...
try:
    ...
    payload.update(
        {
            "Login_Main$UserName": ACCOUNT,
            "Login_Main$Password": PASSWORD,
            "Login_Main$LoginImageButton.x": "30",
            "Login_Main$LoginImageButton.y": "20",
        }
    )
    login_headers = headers.copy()
    login_headers["Referer"] = current_url
    with opener.open(
        urllib.request.Request(
            get_form_action(current_html, current_url),
            data=urllib.parse.urlencode(payload).encode(),
    ...
    print("\n執行：1.單位社選擇...")
    m = re.search(
        r"__doPostBack\('([^']+)','([^']*(?:單位社選擇)[^']*)'\)", current_html
    )
    if m:
        current_html, current_url = do_postback(
            opener,
            get_form_action(current_html, current_url),
            current_html,
            m.group(1),
    ...
    current_html, current_url = click_button(
        opener,
        get_form_action(current_html, current_url),
        current_html,
        "全部選取",
    ...
    ds = re.findall(
        r'<select[^>]+name=["\'](ctl00\$ContentPlaceHolder1\$DropDownList[^"\']+)["\'][^>]*>(.*?)</select>',
        current_html,
        re.I | re.S,
    )
    ovrs = {}
    ...
    for match in re.finditer(
        r'<input[^>]+type=["\']submit["\'][^>]*>', current_html, re.I
    ):
        tag = match.group(0)
        val_m = re.search(r'value=["\']([^"\']*)["\']', tag, re.I)
        if val_m and "資料轉檔(不含累積數)" in val_m.group(1):
            target_btn = re.search(r'name=["\']([^"\']*)["\']', tag, re.I).group(1)
            break
